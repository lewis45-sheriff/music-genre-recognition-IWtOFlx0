...
def main(args):
    ...
    if mode == 'train':
        ...
        plt.subplot(1, 2, 1)
        plt.plot(model_info.history['accuracy'], label='train') 
        plt.plot(model_info.history['val_accuracy'], label='validation')  
        plt.title('Accuracy')
        plt.xlabel('Epochs')
        plt.ylabel('Accuracy')
        plt.legend()
        ...
        classes = ['metal', 'disco', 'classical', 'hiphop', 'country', 'pop', 'blues', 'reggae', 'rock']
        ...
        if args.song is not None:
            ...
            for file in os.listdir(rootdir + "/" + test_folder):
                ...
                if os.path.isfile(path + "/" + file):
                    ...
                    # Run into model
                    results = np.argmax(model.predict(spectrograms), axis=1)
...
def generate_spectrograms(signals, conv_1d=True):
    ...
    for instance in signals:
        # Check if the instance is a tuple of (audio signal, sampling rate)
        if isinstance(instance, tuple) and len(instance) == 2:
            y, sr = instance
        else:
            # If not a tuple, assume it's the audio signal, and set sr to default (22050 Hz)
            y = instance
            sr = 22050  # Default sampling rate, you can adjust this if your data has a different rate

        # Create spectrograms and add a new axis (number of channels = 1 for use in CNN as an image)
        # Librosa creates a spectrogram like (time, frequency)
        # We convert it to (time, frequency, channel)
        # But if we use 1D convolutional layer, no need to add an axis
        if conv_1d:
            rep.append(librosa.feature.melspectrogram(y=y, sr=sr).T)  # Transpose for proper shape
        else:
            rep.append(np.expand_dims(librosa.feature.melspectrogram(y=y, sr=sr).T, axis=2))
